...
function App() {
  ...
  const handleConnectJira = async (e: React.FormEvent) => {
    ...
    if (!jiraConfig.baseUrl || !jiraConfig.email || !jiraConfig.apiKey) {
    ...
    try {
      ...
      const connectRes = await fetch('http://localhost:8090/api/jira/connect', {
        method: 'POST',
  ...
  const fetchStories = async () => {
    try {
      const storiesRes = await fetch('http://localhost:8090/api/jira/stories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error(err)
      setError('Connected to Jira, but failed to fetch stories.')
    }
  }
    ...
    <div>
      <style>{`
        * {
        ...
        .connect-jira-btn.connected {
            background-color: #36B37E; /* Green for connected */
            pointer-events: none;
        }
      ...
      <div className="container">
        <div className="header">
          ...
          <button
            className={`connect-jira-btn ${isJiraConnected ? 'connected' : ''}`}
            onClick={() => !isJiraConnected && setShowJiraModal(true)}
          >
            {isJiraConnected ? '✓ Jira Connected' : 'Connect Jira'}
          </button>
        </div>
          ...
          <div className="modal-overlay" onClick={() => setShowJiraModal(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                ...
                <button className="modal-close" onClick={() => setShowJiraModal(false)}>×</button>
              </div>
              <form onSubmit={handleConnectJira}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={e => handleJiraConfigChange('apiKey', e.target.value)}
                    required
                  />
                </div>
                <button type="submit" className="submit-btn" style={{ width: '100%' }} disabled={isJiraLoading}>
        ...
        <form onSubmit={handleSubmit} className="form-container">
          {/* Jira Story Selection Dropdown */}
          {isJiraConnected && (
            <div className="jira-select-container">
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="form-label">Select User Story from Jira</label>
                <select
